...
# TODO: switch to actual repository, once it's merged
git remote set-url --push origin git@github.com:ilslv/cucumber-rust.git
git config --local user.name "Cucumber Bot"
git config --local user.email "cucumber@example.com"
git add -A $VERSION
